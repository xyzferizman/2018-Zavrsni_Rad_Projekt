...
public class Level5GUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        dbcmd.CommandText = "SELECT * FROM UserLevel WHERE userid=" + GlobalVariables.id + " AND level=5";
        ...
        // sada citamo highscore iz druge tablice
        dbcmd = dbconn.CreateCommand();
        dbcmd.CommandText = "SELECT highscore FROM User WHERE id = " + GlobalVariables.id;
        ownHighscoreText.text = dbcmd.ExecuteScalar().ToString();
        ownHighscore = Int32.Parse(ownHighscoreText.text);
        Debug.Log("own highscore je : " + ownHighscore);
    ...
    private static int GetLossesFallDB(int lossesFallDB)
    {
        return lossesFallDB;
    }
